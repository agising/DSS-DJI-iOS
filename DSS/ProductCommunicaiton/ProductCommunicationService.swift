...
let ProductRegisterOk = "ProductRegisterOk"
...
class ProductCommunicationService: NSObject, DJISDKManagerDelegate {
    ...
    func registerWithProduct() {
        guard
            ...
            appKey != "Set_your_secret_DJISDKKEY_here"
        else {
                print("\n<<<ERROR: Please add DJI App Key in DSS/DSS/Secrets.h after registering as DJI developer. If not visible in your XCODE Navigator, drag it in to the DSS-folder, it is available in finder.>>>\n")
                return
        }
    ...
    // nil if registration is successful. Otherwise it contains an NSError object with error codes from DJISDKRegistrationError.
    func appRegisteredWithError(_ error: Error?) {
        if error == nil {
            self.registered = true
            postNotificationNamed(ProductCommunicationServiceStateDidChange, dispatchOntoMainQueue: true)
            NotificationCenter.default.post(Notification(name: Notification.Name(rawValue: "ProductRegisterOk")))
