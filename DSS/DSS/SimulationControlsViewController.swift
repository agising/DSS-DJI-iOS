...
// No storyboarde, no coordinator. Just keep the DJI layout for simulations control.
class SimulatorControlsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        latitudeTextField.clearButtonMode = .whileEditing
        ...
        self.latitudeControlAction = ControlAction {
            if let lat = self.numberFormatter().number(from: latitudeTextField.text ?? "0") {
        ...
        longitudeTextField.clearButtonMode = .whileEditing
        ...
        self.longitudeControlAction = ControlAction {
            if let long = self.numberFormatter().number(from: longitudeTextField.text ?? "0") {
        ...
        if let priorSimulatorLocation = UserDefaults.standard.value(forKey: UserDefaultsPriorSimulatorLocationKey) as? [String:NSNumber] {
            if let latitudeDegrees = priorSimulatorLocation[UserDefaultsLatitudeKey] {
                ...
                latitudeTextField.text = numberFormatter().string(from:latitudeDegrees)
            }
            
            if let longitudeDegrees = priorSimulatorLocation[UserDefaultsLongitudeKey] {
                self.longitude = longitudeDegrees.doubleValue
                longitudeTextField.text = numberFormatter().string(from:longitudeDegrees)
            }
        }
        ...
        longitudeTextField.centerYAnchor.constraint(equalTo: self.view.centerYAnchor, constant: -50).isActive = true
